...
- **Cosmos Policy Deep-Dive (2026)**: `survey/papers/cosmos-policy-deep-dive.md` — NVIDIA+Stanford ICLR 2026, unified video-policy (Cosmos-Predict2-2B DiT + latent frame injection), LIBERO 98.5% / RoboCasa 67.1% SOTA. **exp09a full sweep**: action-only 659ms/1.5Hz, fixed 265ms + **76.8ms/step** (R²=0.9975), 1-step→342ms/2.9Hz. parallel_gen +698ms. VRAM 8816MB.
...
- **Awesome-WAM Survey (Fudan OpenMOSS, 2605.12090)**: `survey/papers/awesome-wam-survey-2026.md` — WAM 领域首篇系统性 survey, 64 篇论文。硬定义 WAM=`p(o',a|o,l)`, 两大架构族: **Cascaded** (Explicit pixel/Implicit latent) vs **Joint** (AR/Diffusion)。我们已测 4/6 大类, **Cascaded/Implicit + Joint/Multi-DiT 两类未覆盖**。Challenge #5 "inference latency too slow" = 我们的核心方向。无任何 latency benchmark — profiling gap 被学术 survey 正式确认
...
- **Hao meeting prep:** `docs/hao-meeting-prep.md` — 第一次 meeting 大纲 v1 (四范式 + 7 实验)
- **Hao meeting prep v2:** `docs/hao-meeting-prep-v2.md` — 升级版 (10 家 startup landscape + 9 模型 + OFT 瓶颈翻转 + design space 散点图)
