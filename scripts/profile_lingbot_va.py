...
def main():
    ...
    for phase, measurements in times.items():
        if measurements:
            ...
            results[phase] = stats
            print_phase_summary(phase, stats, label_width=16)
    ...
    print(f"  {'TOTAL(e2e)':>16s}: mean={total_mean:7.2f}ms median={total_median:.2f}ms "
          f"p10/p90={total_stats['p10_ms']:.2f}/{total_stats['p90_ms']:.2f} cv={total_stats['cv_pct']:.1f}%")
    print(f"  {'Hz':>16s}: {1000 / total_median:.1f} (from median)")
