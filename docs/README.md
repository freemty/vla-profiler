...
| `survey/papers/starvla-deep-dive.md` | **StarVLA 精读** — HKUST 开源 VLA 训练平台, backbone×action-head 可组合, LIBERO 96.6%, 完全不测 inference efficiency |
| `survey/papers/uva-cosmos-policy-comparative.md` | **UVA vs Cosmos Policy** — 同一组人从 decoupled (0.5B) 到 monolithic (2B) 演进, scale 改变架构选择 |
| `survey/papers/cosmos-policy-deep-dive.md` | **Cosmos Policy 深度** — 2B DiT unified video-policy, latent frame injection, LIBERO 98.5% SOTA, exp09a 实测数据 |
...
| `exp/exp09a/` | Cosmos Policy direct-mode | Done | Action-only 659ms/1.5Hz, per-step 76.8ms |
| `exp/exp11a/` | **OpenVLA-OFT (profiling)** | Planned | Prismatic 7B + parallel MLP OFT. Paper: 109.7Hz A100 |
| `exp/exp11b/` | **StarVLA-OFT (profiling)** | Planned | Qwen3-VL-4B + parallel MLP OFT. LIBERO 96.6%, zero Hz reported |
